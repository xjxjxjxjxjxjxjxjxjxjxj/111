...
/* ============================================================================
 * FILE OVERVIEW (isr_config.h)
 * ----------------------------------------------------------------------------
 * Central interrupt configuration for the TC264. Every interrupt used in the
 * application is assigned:
 *   *_INT_SERVICE : which core (or DMA) services it   -> IfxSrc_Tos_cpu0 /
 *                    IfxSrc_Tos_cpu1 / IfxSrc_Tos_dma
 *   *_INT_PRIO    : priority (1-255, HIGHER number = HIGHER priority).
 *                   If the service is DMA, priority must be 0-47.
 * IMPORTANT: all priorities must be unique (the library enforces this).
 *
 * Used interrupts in this project (see isr.c):
 *   PIT/CCU6 timers (30-33), ERU GPIO ext (40/41/43, ch2+6 via DMA=5),
 *   DMA (60), UART0..3 (10-21).
 * ==========================================================================*/
...
//==========================================CPU1帧任务中断唤醒参数=============================================
// 实车可靠性版默认关闭GPSR00/IDLE，使用原NoRing15的CPU1连续轮询。
#ifndef LOW_COMPUTE_CPU1_INTERRUPT_WAKE
#define LOW_COMPUTE_CPU1_INTERRUPT_WAKE       (0)
#endif

#define CPU1_FRAME_WAKE_INT_SERVICE           IfxSrc_Tos_cpu1
#define CPU1_FRAME_WAKE_INT_PRIO              (70)

// CPU0等待结果时定期检查CPU1状态；若仍处于IDLE，则再次唤醒并强制切回RUN。
#ifndef CPU1_WAKE_FALLBACK_CHECK_MS
#define CPU1_WAKE_FALLBACK_CHECK_MS           (4U)
#endif

#if LOW_COMPUTE_CPU1_INTERRUPT_WAKE && (CPU1_WAKE_FALLBACK_CHECK_MS < 1U)
#error "CPU1_WAKE_FALLBACK_CHECK_MS must be at least 1"
#endif
...
* 日期              作者        内容
* --------------------------------------------------------------------------------------------------------------------
* 2026-08-16       opencode    - 注释整理：文件头部概述中断配置机制(服务核+唯一优先级规则)
*                               - 编码统一：文件由 GBK 转为 UTF-8(带BOM)，中文注释不再乱码
*                               - BUG修复：TASKING编译器不支持UTF-8 BOM，去除文件头BOM(EF BB BF)，
*                                 解决编译报错 E207 token "\xef" deleted (此前44个报错的根因)
* --------------------------------------------------------------------------------------------------------------------
* 说明：本工程实际使用的优先级区间 = PIT(30-33) / ERU(40,41,43; ch2+6经DMA=5)
*       / DMA(60) / UART0-3(10-21)，所有值已互不相同
*********************************************************************************************************************/
