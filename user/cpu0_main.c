...
/*
 * Optional low-rate VOFA+ output. It reuses the original debug UART and does
 * not change the camera, CPU1, PID or motor-control path.
 */
#ifndef VOFA_TELEMETRY_ENABLE
#define VOFA_TELEMETRY_ENABLE          (1)
#endif
#ifndef VOFA_TELEMETRY_FRAME_DIVIDER
#define VOFA_TELEMETRY_FRAME_DIVIDER   (10U)
#endif
#define VOFA_CHANNEL_COUNT             (20U)
#define VOFA_JUSTFLOAT_TAIL            (0x7F800000U)

#if VOFA_TELEMETRY_ENABLE && (VOFA_TELEMETRY_FRAME_DIVIDER < 1U)
#error "VOFA_TELEMETRY_FRAME_DIVIDER must be at least 1"
#endif
...
#define OUTER_SPEED_GAIN_PERCENT       (10)
...
#define GAP_REFERENCE_CENTER_RANGE      (40)
#define GAP_SECOND_REFERENCE_CENTER_RANGE (40)
#define GAP_CENTER_SUM_TARGET           (IMG_COL)
#define GAP_CENTER_SUM_OFFSET_MIN       (20)
#define GAP_LOST_COUNT_MIN              (4)
#define GAP_LOST_RUN_MIN                (3)
#define GAP_OPPOSITE_VALID_MIN          (8)
#define GAP_CONFIRM_FRAMES              (4)
#define GAP_SECOND_CONFIRM_FRAMES       (3)
...
#define GAP_OPPOSITE_EDGE_TOLERANCE     (45)
...
#define RING_WAIT_REFERENCE_LOST_MAX    (8)
 ...
 * MOTOR1 is the physical left wheel and MOTOR2 is the physical right wheel.
 */
#define DIR_R1                         (P02_6)
...
/* Both direction inputs on the new car use LOW for forward and HIGH for reverse. */
#define LEFT_FORWARD_LEVEL             (GPIO_LOW)
#define LEFT_REVERSE_LEVEL             (GPIO_HIGH)
#define RIGHT_FORWARD_LEVEL            (GPIO_LOW)
#define RIGHT_REVERSE_LEVEL            (GPIO_HIGH)
...
#if VOFA_TELEMETRY_ENABLE
/* Signed values describe the command actually written to the two motors. */
static int16 motor_steering_command_signed = 0;
static int16 motor_left_command_percent = 0;
static int16 motor_right_command_percent = 0;
static int16 motor_left_command_duty = 0;
static int16 motor_right_command_duty = 0;
#endif
...
typedef struct
{
    float32 channel[VOFA_CHANNEL_COUNT];
    uint32 tail;
} vofa_packet_t;

typedef char vofa_packet_size_must_be_84_bytes[
    (sizeof(vofa_packet_t) == 84U) ? 1 : -1];

static uint32 vofa_frame_count;
static uint16 vofa_frame_divider;

static void vofa_publish_frame (uint16 current_z,
                                uint16 control_z,
                                uint8 line_valid,
                                ...
                                uint8 reference_valid)
{
    ...
    vofa_frame_count++;
    vofa_frame_divider++;
    if(vofa_frame_count != 1U
    && vofa_frame_divider < VOFA_TELEMETRY_FRAME_DIVIDER)
    {
        return;
    }
    vofa_frame_divider = 0U;

    packet.channel[0] = (float32)vofa_frame_count;
    packet.channel[1] = (float32)current_z;
    packet.channel[2] = (float32)control_z;
    packet.channel[3] = (float32)motor_steering_command_signed;
    packet.channel[4] = (float32)motor_left_command_percent;
    packet.channel[5] = (float32)motor_right_command_percent;
    packet.channel[6] = (float32)motor_left_command_duty;
    packet.channel[7] = (float32)motor_right_command_duty;
    packet.channel[8] = (float32)line_valid;
    packet.channel[9] = (float32)both_edges_lost;
    packet.channel[10] = (float32)left_valid_count;
    packet.channel[11] = (float32)right_valid_count;
    packet.channel[12] = (float32)left_max_lost_run;
    packet.channel[13] = (float32)right_max_lost_run;
    packet.channel[14] = (float32)left_edge_average;
    packet.channel[15] = (float32)right_edge_average;
    packet.channel[16] = (float32)reference_boundary_row;
    packet.channel[17] = (float32)sample_start_row;
    packet.channel[18] = (float32)reference_col;
    packet.channel[19] = (float32)reference_valid;
    packet.tail = VOFA_JUSTFLOAT_TAIL;
    (void)debug_send_buffer((const uint8 *)&packet, (uint32)sizeof(packet));
}
#endif
...
#if VOFA_TELEMETRY_ENABLE
    motor_steering_command_signed = 0;
    motor_left_command_percent = 0;
    motor_right_command_percent = 0;
    motor_left_command_duty = 0;
    motor_right_command_duty = 0;
#endif
}
                                           ...
                                           int base_speed_percent)
{
    ...
    if(turn_y)
    {
        motor_steering_command_signed = (int16)(-command);
    }
    else if(turn_z)
    {
        motor_steering_command_signed = (int16)command;
    ...
    motor_left_command_percent = (int16)(left_sign * left_percent);
    motor_right_command_percent = (int16)(right_sign * right_percent);
    motor_left_command_duty = (int16)(left_sign * (int)left_duty);
    motor_right_command_duty = (int16)(right_sign * (int)right_duty);
#endif
...
static uint16 get_dian (const uint8 *image)
{
    ...
    if(average < first_average * 6U / 10U)
    {
        average = first_average * 6U / 10U;
    }
    return average;
}
...
void core0_main (void)
{
...
#if VOFA_TELEMETRY_ENABLE
    uint16 current_reference_boundary_row;
    uint16 current_sample_start_row;
#endif
    ...
    while(1)
    {
...
#if VOFA_TELEMETRY_ENABLE
        current_reference_boundary_row = reference_boundary_row_result;
        current_sample_start_row = sample_start_row_result;
#endif
        ...
        vofa_publish_frame(current_z,
                           ...
                           current_left_edge_average,
                           current_right_edge_average,
                           current_reference_boundary_row,
                           current_sample_start_row,
                           current_reference_col,
                           current_reference_valid);
#endif
