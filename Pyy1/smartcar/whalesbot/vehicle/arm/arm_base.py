...
POSITION_ERROR_THRESHOLD = 0.01  # 位置误差阈值(放宽以匹配编码器精度)
STOP_CHECK_THRESHOLD = 1e-4  # 停止检查阈值（堵转判定：10帧内位置变化<此值）
...
class ArmController:
    ...
    def y_pid_moveto(self, target_pose):
        ...
        return abs(error) <= POSITION_ERROR_THRESHOLD
    ...
    def reset_y(self):
        ...
        while True:
            if self.y_pid_moveto(target):
                logger.info(
                    f"reset_y: 已移动到目标位置，当前移动距离={self.y_pose_now:.4f}，磁感应器数值={self.y_limit_sensor.read()}"
                )
                break
            if self.y_reset_check():
                ...
                self.y_pose_now = 0
                logger.info(
                    f"reset_y: 检测到限位，当前移动距离={self.y_pose_now:.4f}，磁感应器数值={self.y_limit_sensor.read()}"
                )
                break
            # logger.debug(
            #     f"reset_y: 当前移动距离={self.y_pose_now:.4f}，磁感应器数值={self.y_limit_sensor.read()}"
            # )
        self.y_speed(0)

    def move_y_position(self, target, out_time=16.0):
        """
        以绝对高度值移动竖直方向到目标位置。
        ...
        Args:
            ...
            out_time: 最大允许运动时间
        """
        # print(f"move_y_position: 开始移动到高度 {target}")
        self.y_pid.setpoint = target
        self.y_pid.reset()
        self.y_pose_last = self.y_get_position()
        self.y_distance_change = 0
        end_time = time.time() + out_time

        while True:
            if time.time() > end_time:
                logger.info(f"移到高度{target}超时")
                print(f"move_y_position: 移动到高度 {target} 超时")
                break

            if self.y_pid_moveto(target):
                self.y_speed(0)
                self.y_pose_now = self.y_get_position()
                if abs(target - self.y_pose_now) <= POSITION_ERROR_THRESHOLD:
                    break

            # 向下运动时检测到限位 → 立即停止
            if target < self.y_pose_now and self.y_reset_check():
                self.y_speed(0)
                self.y_pose_start = self.motor_y.get_dis()
                self.y_pose_now = 0.0
                logger.info(f"move_y_position: 下降中触发限位，已停")
                break
    ...
    def move_x_position(self, target, out_time=6.0):
        ...
        Args:
            ...
            out_time: 最大允许运动时间
        """
        # 零点补偿：目标为0时实际往左多移一点，抵消零点偏差
        # if target == 0:
        #     target = -0.01
        end_time = time.time() + out_time
        self.x_pid.setpoint = target
        self.x_pid.reset()
        self.x_pose_last = self.x_get_position()
        self.x_distance_change = 0
    ...
    def reset_x(self):
        """
        重置水平方向位置（电机匀速转动，当编码器变化量第一次低于阈值时判定堵转）
        """
        target = -0.33
        timeout = 8.0
        STALL_THRESHOLD = 0.00015   # 堵转判定阈值（编码器变化量<此值=堵转），可调
        MIN_LOOP = 20               # 最少循环次数，避免初始0值误触发
        time_start = time.time()
        self.x_pid.output_limits = (-0.04, 0.04)
        self.x_pid.setpoint = target
        loop_cnt = 0
        print(f"[reset_x] 开始, 堵转阈值={STALL_THRESHOLD:.5f}")
        while True:
            loop_cnt += 1
            self.x_pid_moveto(target)
            if loop_cnt > MIN_LOOP and abs(self.x_distance_change) < STALL_THRESHOLD:
                self.x_pose_start = self.motor_x.get_dis()
                self.x_pose_now = 0
                self.x_pose_last = 0
                print(f"[reset_x] 堵转! loop=#{loop_cnt} change={self.x_distance_change:.8f}")
                break
            if time.time() - time_start > timeout:
                self.x_pose_start = self.motor_x.get_dis()
                self.x_pose_now = 0
                self.x_pose_last = 0
                print(f"[reset_x] 超时退出 ({timeout:.0f}s), loop=#{loop_cnt}")
                break
        self.x_speed(0)
        self.x_pid.output_limits = self.x_velocity_limit  # 恢复原始速度限制
        print(f"[reset_x] 结束, pos_now={self.x_pose_now:.4f}")
    ...
    def y_move_to_position(self, target):
        """
        直接移动竖直方向到指定位置。
        ...
        Args:
            target: 目标位置
        """
        self.move_y_position(target)

    def x_move_to_position(self, target):
        """
        直接移动水平方向到指定位置。
