...
class MecanumDriver:
    ...
    timeout=30.0,
        ...
        tolerance = np.array(tolerance, dtype=float)
        max_velocities = np.array(max_velocities, dtype=float)
        ...
        yaw_target = current_position[2] + yaw_error
        ...
        if duration is not None:
            ...
            computed_limits = np.abs(delta) / duration

            # 不让某个轴因为初始误差为0而完全失去纠偏能力
            computed_limits = np.where(
                computed_limits > 0,
                computed_limits,
                max_velocities,
            )

            max_velocities = np.minimum(
                computed_limits,
                max_velocities,
            )

        self.pid_x.setpoint = target_position[0]
        self.pid_x.output_limits = (
            -max_velocities[0],
            max_velocities[0]
        )

        self.pid_y.setpoint = target_position[1]
        self.pid_y.output_limits = (
            -max_velocities[1],
            max_velocities[1]
        )

        self.pid_yaw.setpoint = yaw_target
        self.pid_yaw.output_limits = (
            -max_velocities[2],
            max_velocities[2]
        )
        ...
        try:
            while True:
                ...
                with self._lock:
                    current_position = (
                        self.chassis.odometry.position.copy()
                    )
                ...
                velocity_x_pid = self.pid_x(
                    current_position[0]
                )

                velocity_y_pid = self.pid_y(
                    current_position[1]
                )

                angular_velocity_pid = self.pid_yaw(
                    current_position[2]
                )
    ...
    def move_to_position(
        ...
        tolerance=(0.004, 0.004, 0.02),
        timeout=30.0,  # 添加超时参数
        lateral_enabled=True,  # False=只允许前进后退，禁止侧向与转向
    ):
        """
        通过PID控制移动到指定位姿
        ...
        参数:
            ...
            lateral_enabled: True=麦轮全向移动；False=仅前进后退，禁止侧向与转向
        """
        ...
        while True:
            ...
            # 禁止侧向/旋转时 y、z 轴无法被主动纠正，判停只校验 x，避免卡到超时
            if not lateral_enabled:
                error_within_threshold[1] = True
                error_within_threshold[2] = True
            ...
            if lateral_enabled:
                ...
                # 禁止侧向移动：清零 y-PID 输出并重置积分防止 windup
                self.pid_y.reset()
                velocity_y_pid = 0.0
            ...
            if lateral_enabled:
                ...
                # 禁止旋转：清零 yaw-PID 输出并重置积分防止 windup
                self.pid_yaw.reset()
                angular_velocity_pid = 0.0
            # 世界坐标速度转换车子坐标速度
            velocity_output = self.world_to_car_velocity(
            ...
            # 二次保障：禁止侧向/旋转时强制清零车体系 y、z 速度
            if not lateral_enabled:
                velocity_output[1] = 0.0
                velocity_output[2] = 0.0
