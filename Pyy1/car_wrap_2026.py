...
class MyCar(MecanumDriver):
    ...
    def _get_color_center_direct(self, color, image=None, save_debug=False):
        ...
        return self._get_color_center(color, save_debug=save_debug, image=image)
    ...
    def move_to_detection_target(
        ...
        sort_pos=(0, 0),
        num=0,
        align_y=True,   # True=对准dy, False=跳过dy对准（不动机械臂y）
        pid_params=None,  # 自定义PID参数(可左右分开): {"RIGHT":{...},"LEFT":{...}} 或 {"kp_x":...,"kp_y":...,...}
        arm_step_limit=None,  # 机械臂每次移动(位移=out_y*0.05)的限幅，单位米；None=不限幅
    ):
        time_stop = time.time() + time_out
        x_count = CountRecord(2)
        ...
        # 允许调用方传入自定义 PID 参数覆盖默认值，支持两种写法：
        #   1) 左右分开（推荐）: pid_params={"RIGHT": {"kp_x":..., "kp_y":...}, "LEFT": {...}}
        #   2) 不分左右:        pid_params={"kp_x":..., "kp_y":...}  （左右都用同一套）
        if pid_params:
            side_key = "RIGHT" if self.arm.side == "RIGHT" else "LEFT"
            # 嵌套写法：取当前机械臂所在侧的那一套；没写该侧或没写某键则沿用默认值
            params = pid_params[side_key] if isinstance(pid_params.get(side_key), dict) else pid_params
            kp_x = params.get("kp_x", kp_x)
            ki_x = params.get("ki_x", ki_x)
            kd_x = params.get("kd_x", kd_x)
            kp_y = params.get("kp_y", kp_y)
        ...
        while True:
            ...
            if len(dets) > num:
                ...
                if delta_y is None or not align_y:
                    out_y = 0
                else:
                    out_y = kp_y * (dy - delta_y)
            ...
            if arm_step_limit is not None:
                out_y = max(-arm_step_limit, min(arm_step_limit, out_y * 0.05)) / 0.05
            self.arm.x_speed(out_y)
            time.sleep(0.05)
    ...
    def move_to_wt3(
        ...
        save_images=1,     # 0=不保存图片, 1=保存调试图片
    ):
        """手动PID版：不依赖外部PID库，所有参数在函数内部直接可调。
        color_mode: 当 use_hsv=True 时选择颜色模式:
            1 = 浅蓝色 (默认, 水块)
            2 = 深蓝 + 黄色 (汉诺塔)
        save_images: 0=不保存调试图, 1=保存HSV过程图和对齐图(默认)"""
